...
//! were `3`, `9`, and `6`, your device's built-in adapter would be rated for
//! `12` jolts.)
//!
...
//!   rating of `1`, `2`, or `3` jolts. Of these, only one you have is an
...
//!   pick the adapter rated `5` jolts (difference of `1`).
...
fn build_sorted(array: &[usize]) -> Vec<usize> {
...
/// TODO: Try a scala collection style `groupBy` to count hits.
fn count_differences(sorted_array: &[usize]) -> (usize, usize, usize) {
    sorted_array
        .windows(2)
        .fold((0, 0, 0), |acc, pair| match pair[1] - pair[0] {
...
fn count_viable_routes(sorted_array: &[usize]) -> usize {
    let set = sorted_array.iter().collect::<HashSet<&usize>>();
    let max = *sorted_array.last().unwrap();
    let mut counter: Vec<usize> = vec![0; max + 1];
    counter[0] = 1;

    for &elem in sorted_array.iter().take(sorted_array.len() - 1) {
        for inc in 1..=3 {
            if set.get(&(elem + inc)).is_some() {
                counter[elem + inc] += counter[elem]
            }
        }
    }
    *counter.last().unwrap()
...
fn main() -> Result<(), &'static str> {
    ...
    let sorted_array = build_sorted(&array);

    let diff_count = count_differences(&sorted_array);
    println!(
        "The number of 1-jolt differences multiplied by the number of 3-jolt differences is {}.",
        diff_count.0 * diff_count.2
    );

    let way_count = count_viable_routes(&sorted_array);
    println!(
        "The total number of distinct ways you can connect is {}.",
        way_count
    );
...
mod tests {
    use super::*;
    const TEST_ARRAY: &[usize] = &[16, 10, 15, 5, 1, 11, 7, 19, 6, 12, 4];

    #[test]
    fn test_count_differences() {
        let sorted = build_sorted(TEST_ARRAY);
        let diff_count = count_differences(&sorted);
        assert_eq!(diff_count.0, 7);
        assert_eq!(diff_count.2, 5);
    }

    #[test]
    fn test_count_viable_routes() {
        let sorted = build_sorted(TEST_ARRAY);
        let num = count_viable_routes(&sorted);
        assert_eq!(num, 8);
    }
}
